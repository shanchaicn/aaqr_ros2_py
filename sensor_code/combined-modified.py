...
try:
    ...
    logging.warning(f"LTR559 initialisation failed: {e}")
except ImportError:
    import ltr559
...
# Raspberry Pi4 IP
server_ip = "http://192.168.8.101:5000/send-data"
    ...
    try:
        while True:
            ...
            if mode == 10:
                ...
                cpu_temps = cpu_temps[1:] + [cpu_temp]
                avg_cpu_temp = sum(cpu_temps) / float(len(cpu_temps))
#                res["Average CPU Temp"] =  avg_cpu_temp
                raw_temp = bme280.get_temperature()
                ...
                res["pressure"] =  raw_data
                save_data(1, raw_data)
                display_everything()
                raw_data = bme280.get_humidity()
                res["humidity"] =  raw_data
                ...
                res["oxidising"] = oxidising
                save_data(5, gas_data.reducing / 1000)
                reducing = gas_data.reducing / 1000
                res["reducing"] = reducing
                ...
                try:
                    ...
                    res["pm1"] = pms_data.pm_ug_per_m3(1.0)
                    save_data(8, float(pms_data.pm_ug_per_m3(2.5)))
                    res["pm2.5"] = pms_data.pm_ug_per_m3(2.5)
                    save_data(9, float(pms_data.pm_ug_per_m3(10)))
                    res["pm10"] = pms_data.pm_ug_per_m3(10)
                    display_everything()
                    data = res
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            logging.error(f"Failed to send post request. Error code: {response.status_code}")
